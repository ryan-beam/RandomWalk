...
def random_walk_2d(n_steps=1000):
    ...
    steps = np.random.choice([-1, 1], size=(n_steps, 2))  # Random ±1 in x,y
    path = np.cumsum(steps, axis=0)                        # Convert steps to positions
    return np.vstack([[0, 0], path])                       # Prepend origin
    ...
    def update(frame):
        ...
        if frame > 0:
            seg, = ax.plot(path[frame-1:frame+1, 0], path[frame-1:frame+1, 1],
                          color=plt.cm.viridis(colors[frame]), alpha=0.7, linewidth=2)
            line_segments.append(seg)
        # Update end marker position
        end_marker.set_offsets([path[frame, 0], path[frame, 1]])
        ax.set_title(f'2D Random Walk (step {frame}/{len(path)-1})')
...
if __name__ == "__main__":
    path = random_walk_2d(n_steps=500)  # Fewer steps for smoother animation
